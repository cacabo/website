...
if (!SENDGRID_API_KEY) throw new Error('Missing SENDGRID_API_KEY in env')
...
// Freelancing
router.get('/freelancing', (_, res) =>
  res.render('freelancing', {
    title: 'Cameron Cabo | Freelancing',
    isFreelancingActive: true,
    description:
      'I develop websites and mobile apps for all kinds of small businesses and startups. Through my work I hope to grow and help others do the same. Contact me and we can get to work.',
  })
)
...
router.get('/projects', (_, res) =>
  res.render('projects', {
    ...
    description:
      'I am a firm believer in learning by doing. Building projects is my means of picking up new tech and better understanding technology, users, and markets.',
    projects,
...
// Art and design
router.get('/art-design', (_, res) =>
  res.render('artDesign', {
    title: 'Cameron Cabo | Art/Design',
    description:
      "Ever since preschool I have loved to draw and create. I approach every challenge, job, and goal with an artist's mindset in order to build impactful and well-crafted products.",
    art,
    design,
...
router.get('/contact', (_, res) =>
  res.render('contact', {
    ...
    description:
      'Want to learn more about my experience or interests? Have a project of your own? Please reach out and I will get back to you as soon as possible.',
...
router.get('/posts/:slug', (req, res) => {
  ...
  if (!postObj || !postObj.post) {
    // The slug was not matched, render the not found route
...
router.post('/contact', (req, res) => {
  ...
  let error = ''
  if (!EMAIL) error = 'EMAIL not found'
  else if (!SENDGRID_API_KEY) error = 'SENDGRID_API_KEY not found'
  if (error) {
  ...
  if (!firstName) error = 'First name must be populated'
    ...
    error = 'First name limited to 100 characters'
...
router.get('*', (_, res) => renderNotFound(res))
